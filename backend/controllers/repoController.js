...
async function commitToRepository(req, res) {
  ...
  // Get 'content' (file content) from body along with message and userId
  const { message, userId, content } = req.body;
  try {
    if (!message) { return res.status(400).json({ error: "Commit message is required." }); }
    if (!mongoose.Types.ObjectId.isValid(userId)) { return res.status(400).json({ error: "Invalid User ID format provided." }); }
    const authorUser = await User.findById(userId);
    if (!authorUser) { return res.status(404).json({ error: "Commit author (user) not found." }); }

    const repository = await Repository.findById(id);
    if (!repository) { return res.status(404).json({ error: "Repository not found!" }); }

    // Add check: only owner can commit? Assumes middleware adds req.user.id
    // if (repository.owner.toString() !== req.user?.id) {
    //     return res.status(403).json({ error: "Forbidden: Only owner can commit to this repository." });
    // }

    // Add commit message to history
    const newCommit = { message: message, author: userId, timestamp: new Date() };
    repository.content.push(newCommit);

    // Update the latestContent field with the new content
    // Use an empty string if content is not provided in the request
    repository.latestContent = content || "";

    const updatedRepository = await repository.save();
    // Return only necessary info, maybe not the full repo?
    res.status(200).json({ message: "Commit successful!", commit: newCommit });
...
async function getAllRepositories(req, res) {
  try {
    // Note: This fetches ALL repos, public and private. Use getPublicRepositories for Explore page.
...
async function fetchRepositoryById(req, res) {
  ...
  try {
    ...
    // Select the 'latestContent' field
    const repository = await Repository.findById(id)
      .populate("owner", "username _id") // Also populate owner ID
      .select('name description visibility owner createdAt content latestContent'); // Add latestContent
...
async function toggleVisibilityById(req, res) {
  ...
  try {
    ...
    const repository = await Repository.findById(id);
    if (!repository) { return res.status(404).json({ error: "Repository not found!" }); }
    // Add check: only owner can toggle? Assumes middleware adds req.user.id
    // if (repository.owner.toString() !== req.user?.id) {
    //     return res.status(403).json({ error: "Forbidden: Only owner can change visibility." });
    // }
    repository.visibility = !repository.visibility;
    const updatedRepository = await repository.save();
...
async function deleteRepositoryById(req, res) {
  ...
  try {
    ...
    if (!repository) { return res.status(404).json({ error: "Repository not found!" }); }
    const ownerId = repository.owner;
    // Add check: only owner can delete? Assumes middleware adds req.user.id
    // if (ownerId.toString() !== req.user?.id) {
    //     return res.status(403).json({ error: "Forbidden: Only owner can delete repository." });
    // }
    const deleteResult = await Repository.findByIdAndDelete(id);
